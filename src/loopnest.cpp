...
long long Loopnest::volume_at_level(Array& arr, int start_loop_level) {
...
long long Loopnest::iters_at_level(int start_loop_level) {
  std::vector<int> extent;
  get_extent(extent,start_loop_level);

  long long iters=1;
  for(long long i = 0; i < extent.size(); i++) {
    iters*=extent[i];
  }
  return iters;
}
...
float Loopnest::bandwidth_for_cache(int datatype_bytes, int cache_bytes, 
  ...
  long long iters = iters_at_level(lvl);
